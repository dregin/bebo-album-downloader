...
def get_album_links(num_album_pages, albums_page, br):
	for x in range (1, num_album_pages+1):									# For each album page
		...
		for i in album_links:
			...
			album_names.append(smart_str(album_name))
...
def get_image_links(album_link, album_name):
	...
	album_directory = smart_str(album_directory)
	...
	#		print e
	#	print "Done!"
	#	Find the o.jpg strings in the javascript at the bottom of the page
	#	build http://file.....a.bebo.com/.......
	regex = re.compile(r"file\d+/*\d*/original/\d+/\d+/\d+/\d+/*\d+[a-z]\d+[a-z]*\d+o\.jpg")
	image_links = regex.findall(str(soup))
	print "Number of images:" + str(len(image_links))
	#	Find the letter before ".bebo.com" to help build the link properly.
	#	Find the second <img> tag and use it's src value to find thttp://www.breakingnews.ie/ireland/social-media-users-not-exempt-from-defamation-laws-458964.htmlhe letter.
	img_src_links = soup.body.findAll('img', {'src':re.compile(".+m.jpg$")})
	img_src_link = img_src_links[0]['src']
	img_src_rem_end = re.sub(r'\.bebo\.com.*', '', img_src_link)
	img_src_rem_start = re.sub(r'http://file\d+', '', img_src_rem_end)
	# img_src_rem_end
	album_name_soup = soup.find("h2", id="album_name")
	album_name_rem_end = re.sub(r' <span>.*', '', str(album_name_soup))
	album_name_rem_start = re.sub('<h2 id=\"album_name\">', '', album_name_rem_end)
	
	album_directory = album_name_rem_start.replace('/', ' ')
	album_directory = album_directory.replace('&#39;', '\'')
	album_directory = smart_str(album_directory)

	# print "ALBUM IS: " + album_directory
	...
	if not os.path.isdir(album_path):
		...
		try:
			...
			print e
		print "Done!"
	# print img_src_rem_start
	for x in image_links:
		x = smart_str(x)
		split_link = x.split("/", 1)
		img_link_url = "http://" + split_link[0] + img_src_rem_start + ".bebo.com/" + split_link[1]
		# print img_link_url
		download_image(img_link_url, album_path) 
	return 0

def download_image(url, directory):
	opener = urllib.FancyURLopener(proxy)
	webFile = opener.open(url)
	print directory + smart_str(url).split('/')[-1]
	localFile = open(directory + smart_str(url).split('/')[-1], 'w')
...
if __name__ == '__main__':
	...
	zip_folder = r"./bebo_pix/" + username + '/'
...
#	os.system("rm -rf ./bebo_pix/" + username)
